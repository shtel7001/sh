...
type Stock={name:string;code:string;market:'KOSDAQ';rank?:number;currentPrice:number|null;changePct:number|null;marketCap:number|null;sector:string|null;theme:string|null};
...
export default function KosdaqDashboard(){
  ...
  useEffect(()=>{try{const r=localStorage.getItem('kosdaq_all_spike_240_rows');if(r)setRows(JSON.parse(r));const u=localStorage.getItem('kosdaq_all_spike_240_universe');if(u){const x=JSON.parse(u);if(Date.now()-x.ts<3*3600000){setUniverse(x.stocks);setUniverseCount(x.stocks.length)}}}catch{}},[]);

  async function loadUniverse(){
    if(universe.length)return universe;
    setStatus('1/3 · 네이버 금융 KOSDAQ 전 종목 목록 수집 중…');
    const r=await fetch('/api/universe',{cache:'no-store'}),j=await r.json();if(!r.ok)throw new Error(j.error||'종목목록 수집 실패');
    const stocks=(j.stocks||[]) as Stock[];if(stocks.length<1000)throw new Error(`KOSDAQ 종목이 ${stocks.length}개만 수집되어 중단했습니다.`);
    setUniverse(stocks);setUniverseCount(stocks.length);localStorage.setItem('kosdaq_all_spike_240_universe',JSON.stringify({ts:Date.now(),stocks}));return stocks;
  }
  ...
  async function scanToday(){
    const today=kst();setStartDate(today);setEndDate(today);setPeriod(1);setLiveMode(true);setBusy(true);setRows([]);setProgress({done:0,total:0,ok:0,fail:0});setStatus('당일 실시간 · 네이버 금융 KOSDAQ 현재가 전 종목 조회 중…');
    try{
      ...
      setRows([...found].sort((a,b)=>b.maxRisePct-a.maxRisePct));found=await enrich(found);found.sort((a,b)=>b.maxRisePct-a.maxRisePct);setRows(found);localStorage.setItem('kosdaq_all_spike_240_rows',JSON.stringify(found));
      setStatus(`완료 · 당일 현재가 기준 KOSDAQ ${count}종목 · +${threshold.toFixed(1)}% 이상 ${found.length}종목 · 기준일 ${j.quoteDate||today}`);
  ...
  async function scan(){
    ...
    try{
      ...
      found=await enrich(found);found.sort((a,b)=>b.maxRisePct-a.maxRisePct);setRows(found);localStorage.setItem('kosdaq_all_spike_240_rows',JSON.stringify(found));
      setStatus(`완료 · KOSDAQ ${stocks.length}종목 분석 · 조건 통과 ${found.length}종목 · 정상 ${ok} · 오류 ${fail}`);
  ...
  function excel(){
    ...
    XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet([['KOSDAQ 전종목 급등 스캐너'],['검색일',kst()],['검색모드',liveMode?'당일 실시간':'과거 일봉'],['날짜범위',`${startDate} ~ ${endDate}`],['최대 거래일',period],['하루 상승률 기준(%)',threshold],['KOSDAQ 종목수',universeCount],['조건 통과',rows.length],['당일 현재가','네이버 금융'],['과거 일봉','Yahoo Finance'],['테마','인포스탁 우선 · 미확인 시 네이버 기업정보 보조']]),'요약');
    ...
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(detail),'급등상세');XLSX.writeFile(wb,`KOSDAQ_전종목_급등_${kst()}_${liveMode?'당일실시간':period+'일'}_${threshold}pct.xlsx`);
  }
  const pct=progress.total?Math.min(100,progress.done/progress.total*100):0;
  return <div className="kqapp"><header className="kqtop"><div className="kqlogo">KQ</div><div><div className="kqtitle">KOSDAQ 전종목 240일 급등 스캐너</div><div className="kqsub">전 종목 · 당일 실시간 · 최대 240거래일</div></div><button className="logout" onClick={logout}>인증 해제</button></header>
    <main className="kqwrap"><section className="kqcard"><div className="eye">01 · 조건 설정</div><h1 className="hero">원하는 날짜에서<br/><em>하루 급등</em> 전부 찾기</h1><hr/>
      ...
      <div className="stats"><div><b>{period}</b><span>최대 거래일</span></div><div><b>{universeCount||'전체'}</b><span>KOSDAQ 종목</span></div><div><b>{liveMode?'LIVE':'1일'}</b><span>{liveMode?'당일 현재가':'급등 판정'}</span></div></div>
      <button className="go" onClick={scan} disabled={busy}>{busy?'검색 중…':'KOSDAQ 전종목 검색 →'}</button><div className="bar"><i style={{width:`${pct}%`}}/></div><div className="status">{status}</div>
      <p className="note"><b>당일 실시간</b>은 네이버 금융 KOSDAQ 현재가/등락률을 캐시 없이 다시 조회합니다. 과거 검색은 선택한 날짜 구간 안에서 최대 240거래일의 Yahoo Finance 일봉을 분석합니다.</p>
    </section>
    <section className="kqcard"><div className="head"><div><div className="eye">02 · 검색 결과</div><h2>{shown.length}개 종목 발견</h2><div className="cond">{liveMode?`당일 실시간 · 기준일 ${quoteDate||endDate}`:`${startDate} ~ ${endDate} · 최대 ${period}거래일`} · +{threshold.toFixed(1)}% 이상</div></div><div className="tools"><select value={sort} onChange={e=>setSort(e.target.value as any)}><option value="rise">최대 급등률순</option><option value="recent">최근 급등일순</option><option value="hits">포착횟수순</option><option value="rank">시총순위순</option></select><button onClick={excel} disabled={!rows.length}>엑셀 저장</button></div></div>
      <div className="diag"><div><b>{universeCount}</b><span>KOSDAQ 종목</span></div><div><b>{progress.ok}</b><span>{liveMode?'현재가 정상':'Yahoo 정상'}</span></div><div><b>{progress.fail}</b><span>데이터 오류</span></div></div>
