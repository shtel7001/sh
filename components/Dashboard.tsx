...
type Stage = { state: StageState; count: number; message?: string };
type SortMode = 'score' | 'spike' | 'volume';

const STAGES = 10;
const CHUNK = 50;
const emptyStages = (): Stage[] => Array.from({ length: STAGES }, () => ({ state: 'idle', count: 0 }));
...
const naverStock = (code: string): string => `https://finance.naver.com/item/main.naver?code=${code}`;
const naverNews = (name: string): string => `https://search.naver.com/search.naver?where=news&query=${encodeURIComponent(name)}`;
const yahoo = (code: string): string => `https://finance.yahoo.com/quote/${code}.KS/`;
...
export default function Dashboard() {
  ...
  const [status, setStatus] = useState<string>('대기 중 · KOSPI 시가총액 상위 500종목을 50개씩 10단계로 분석합니다.');
  const [query, setQuery] = useState<string>('');
  const [lookbackDays, setLookbackDays] = useState<number>(60);
  const [spikePct, setSpikePct] = useState<number>(5);
  const [newsLeadDays, setNewsLeadDays] = useState<number>(3);
  const [candidateMin, setCandidateMin] = useState<number>(55);
  ...
  const failedStages = stages.reduce<number[]>((acc, s, i) => {
    if (s.state === 'failed') acc.push(i + 1);
    return acc;
  }, []);

  const candidateRows = useMemo<RadarRow[]>(() => {
    return rows
      .filter((r) => r.ok && r.score >= candidateMin && (r.latestChangePct ?? 0) < spikePct)
      .slice()
      .sort((a, b) => b.score - a.score || (b.volumeRatio ?? 0) - (a.volumeRatio ?? 0));
  }, [rows, candidateMin, spikePct]);

  const visibleRows = useMemo<RadarRow[]>(() => {
    const q = query.trim().toLowerCase();
    const out = rows.filter((r) => !q || r.name.toLowerCase().includes(q) || r.code.includes(q)).slice();
    if (sortMode === 'spike') out.sort((a, b) => b.spikeCount - a.spikeCount || b.score - a.score);
    else if (sortMode === 'volume') out.sort((a, b) => (b.volumeRatio ?? 0) - (a.volumeRatio ?? 0));
    else out.sort((a, b) => b.score - a.score || b.spikeCount - a.spikeCount);
    return out;
  }, [rows, query, sortMode]);

  function updateStage(index: number, patch: Partial<Stage>): void {
    setStages((prev) => prev.map((s, i) => i === index ? { ...s, ...patch } : s));
  }

  async function loadUniverse(): Promise<UniverseStock[]> {
    setStatus('네이버 금융 기준 KOSPI 시가총액 상위 종목을 수집하는 중…');
    const res = await fetch('/api/radar/universe', { cache: 'no-store' });
    const data: any = await res.json();
    if (!res.ok) throw new Error(String(data?.error || 'KOSPI 500 종목목록 수집 실패'));
    const stocks = (Array.isArray(data?.stocks) ? data.stocks : []) as UniverseStock[];
  ...
  async function runOneStage(stageIndex: number, stocks: UniverseStock[]): Promise<boolean> {
    const batch = stocks.slice(stageIndex * CHUNK, (stageIndex + 1) * CHUNK);
    if (!batch.length) {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      updateStage(stageIndex, { state: 'running', count: 0, message: attempt === 0 ? '분석 중' : `재시도 ${attempt}/2` });
      try {
        const res = await fetch('/api/radar/batch', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ stocks: batch, lookbackDays, spikePct, newsLeadDays }),
        });
        const data: any = await res.json();
        if (!res.ok) throw new Error(String(data?.error || `HTTP ${res.status}`));
        const got = (Array.isArray(data?.rows) ? data.rows : []) as RadarRow[];
        setRows((prev) => {
          const map = new Map<string, RadarRow>(prev.map((x) => [x.code, x]));
          for (const item of got) map.set(item.code, item);
          return Array.from(map.values());
        });
        const okCount = got.filter((x) => x.ok).length;
        const sec = data?.meta?.elapsedMs ? `${(Number(data.meta.elapsedMs) / 1000).toFixed(1)}초` : '완료';
        updateStage(stageIndex, { state: 'done', count: okCount, message: sec });
        return true;
      } catch (error) {
        if (attempt < 2) {
          updateStage(stageIndex, { state: 'running', count: 0, message: `오류 · ${attempt + 1}차 재시도 대기` });
          await delay(1000 * (attempt + 1));
        } else {
          updateStage(stageIndex, { state: 'failed', count: 0, message: error instanceof Error ? error.message : '분석 실패' });
        }
      }
    }
    return false;
  ...
  async function start(reset: boolean): Promise<void> {
    if (running) return;
    setRunning(true);
    ...
    try {
      const stocks = universeRef.current.length ? universeRef.current : await loadUniverse();
      for (let i = 0; i < STAGES; i += 1) {
        if (stopRef.current) break;
        setStatus(`${i + 1}/10단계 분석 중 · ${i * 50 + 1}~${Math.min((i + 1) * 50, stocks.length)}위`);
        await runOneStage(i, stocks);
      }
      setStatus(stopRef.current ? '분석을 중지했습니다.' : '분석 완료 · 실패 구간이 있으면 해당 구간만 다시 시도할 수 있습니다.');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : '분석 중 오류가 발생했습니다.');
    } finally {
      setRunning(false);
    }
  }
  ...
  async function retryFailed(): Promise<void> {
    if (running || failedStages.length === 0) return;
    setRunning(true);
    stopRef.current = false;
    try {
      const stocks = universeRef.current.length ? universeRef.current : await loadUniverse();
      for (const n of failedStages) {
        ...
        await runOneStage(n - 1, stocks);
      }
      setStatus('미수집 구간 재분석을 마쳤습니다.');
    } catch (error) {
      setStatus(error instanceof Error ? error.message : '재분석 중 오류가 발생했습니다.');
    } finally {
      setRunning(false);
    }
  }
  ...
  async function exportExcel(): Promise<void> {
    ...
    const flat = visibleRows.map((r) => ({
      ...
      삼일모멘텀: r.momentum3Pct ?? '',
      오일모멘텀: r.momentum5Pct ?? '',
      MA5: r.ma5 ?? '',
      MA20: r.ma20 ?? '',
      급등횟수: r.spikeCount,
      관찰점수: r.score,
  ...
  return (
    <main className="appShell">
      <header className="topbar">
        ...
        <div className="brandCopy"><b>코스피 뉴스 급등 레이더 V2</b><span>NEWS × PRICE × VOLUME</span></div>
        <button className="ghostBtn" onClick={() => { void start(true); }} disabled={running}>⌕ 지금 새로 분석</button>
      </header>
      ...
      <section className="stagePanel">
        <div className="stageHead">
          ...
          {running ? <button className="warnBtn" onClick={stop}>중지</button> : <button className="miniBtn" onClick={() => { void start(false); }}>이어 분석</button>}
        </div>
        <div className="stageGrid">
          {stages.map((s, i) => (
            <div key={i} className={`stage ${s.state}`} title={s.message || ''}>
              <span>{i + 1}</span><b>{s.state === 'done' ? '완료' : s.state === 'running' ? '진행' : s.state === 'failed' ? '실패' : '대기'}</b><small>{s.count || ''}</small>
            </div>
          ))}
        </div>
        {failedStages.length > 0 && (
          <div className="retryBox"><div><b>미수집 구간 {failedStages.join(', ')}단계</b><span>다른 구간 결과는 유지한 채 실패 구간만 다시 수집합니다.</span></div><button onClick={() => { void retryFailed(); }} disabled={running}>미수집만 재시도</button></div>
        )}
      </section>
      ...
      <section className="candidateSection">
        ...
        {candidateRows.length === 0 ? <div className="emptyState">분석을 시작하면 조건에 맞는 후보가 여기에 표시됩니다.</div> : (
          <div className="candidateGrid">
            {candidateRows.slice(0, 20).map((r, idx) => (
              <article className="candidateCard" key={r.code}>
                ...
                <div className="metricRow"><span>등락 <b className={(r.latestChangePct ?? 0) >= 0 ? 'up' : 'down'}>{fmtPct(r.latestChangePct)}</b></span><span>거래량 <b>{r.volumeRatio?.toFixed(2) ?? '-'}배</b></span><span>최근뉴스 <b>{r.recentNewsCount}건</b></span></div>
      ...
      <section className="resultsSection">
        ...
        <div className="toolbar">
          ...
          <select value={sortMode} onChange={(e) => setSortMode(e.target.value as SortMode)}><option value="score">점수순</option><option value="spike">급등횟수순</option><option value="volume">거래량순</option></select>
          <button onClick={() => { void exportExcel(); }} disabled={!rows.length}>엑셀 저장</button>
        </div>
        <div className="tableWrap">
          <table>
            ...
            <tbody>
              {visibleRows.map((r) => (
                <tr key={r.code} className={!r.ok ? 'errorRow' : ''}>
                  <td><b>{r.name}</b><small>{r.code} · #{r.rank}</small></td>
                  <td>{r.ok ? <><span className={(r.latestChangePct ?? 0) >= 0 ? 'up' : 'down'}>{fmtPct(r.latestChangePct)}</span><small>거래량 {r.volumeRatio?.toFixed(2) ?? '-'}배 · 3일 {fmtPct(r.momentum3Pct)}</small></> : <span className="errorText">{r.error || '수집 실패'}</span>}</td>
              ...
              {visibleRows.length === 0 && <tr><td colSpan={6}><div className="emptyState small">아직 분석 결과가 없습니다.</div></td></tr>}
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section className="launchPanel">
        ...
        <button className="primaryBtn" onClick={() => { void start(true); }} disabled={running}>{running ? '분석 진행 중…' : 'KOSPI 500 분석 시작'}</button>
      </section>

      <footer><p>개인용 데이터 분석 도구 · 뉴스 선행 여부는 수집 가능한 기사 날짜를 기준으로 계산되며 뉴스 누락 가능성이 있습니다.</p><form action="/api/auth/logout" method="post"><button>로그아웃</button></form></footer>
    </main>
  );
}
