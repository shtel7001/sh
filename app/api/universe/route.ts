...
type Stock={name:string;code:string;market:'KOSDAQ';currentPrice:number|null;changePct:number|null;marketCap:number|null;sector:null;theme:null};
...
async function pcUniverse(){ const out:Stock[]=[]; for(let page=1;page<=45;page++){ const html=await pcHtml(`https://finance.naver.com/sise/sise_market_sum.naver?sosok=1&page=${page}`); const $=cheerio.load(html); let count=0; $('table.type_2 tr, table.type2 tr').each((_,tr)=>{ const a=$(tr).find('a.tltle, a[href*="/item/main.naver?code="]').first(); if(!a.length)return; const name=a.text().trim(), code=((a.attr('href')||'').match(/code=(\d{6})/)||[])[1]; if(!code||!name)return; const cells=$(tr).find('td').map((_,td)=>$(td).text().replace(/\s+/g,' ').trim()).get(); out.push({name,code,market:'KOSDAQ',currentPrice:n(cells[2]||''),changePct:n(cells[4]||''),marketCap:n(cells[6]||''),sector:null,theme:null}); count++; }); if(count===0)break; } return out; }
...
export async function GET(){
 ...
 if(!stocks.length)return NextResponse.json({error:'코스닥 종목목록 수집 실패',errors,stocks:[]},{status:502});
