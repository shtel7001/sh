...
type LiveStock={name:string;code:string;market:'KOSDAQ';rank?:number;currentPrice:number|null;changePct:number|null;marketCap:number|null;sector:null;theme:null;currentVolume:number;quoteDate:string};
...
async function page(p:number){
  const url=`https://m.stock.naver.com/api/stocks/marketValue/KOSDAQ?page=${p}&pageSize=100&_=${Date.now()}`;
  let last='';
  for(let a=0;a<3;a++){
...
function norm(x:any):Omit<LiveStock,'rank'>|null{
  ...
  const name=String(x?.stockName??x?.name??x?.itemName??'').trim();if(!code||!name)return null;
  const traded=String(x?.localTradedAt??x?.tradedAt??x?.tradeDate??x?.date??'');const quoteDate=traded.match(/\d{4}-\d{2}-\d{2}/)?.[0]||kst();
  return {name,code,market:'KOSDAQ',currentPrice:n(x?.closePrice??x?.currentPrice??x?.nowVal??x?.price),changePct:n(x?.fluctuationsRatio??x?.changeRate??x?.changePct??x?.rate),marketCap:n(x?.marketValue??x?.marketCap??x?.marketValueAmount),sector:null,theme:null,currentVolume:n(x?.accumulatedTradingVolume??x?.volume??x?.tradingVolume)??0,quoteDate};
}
export async function POST(req:Request){
  if(!await isAuthed())return unauthorized();
  const body=await req.json().catch(()=>({}));const threshold=Math.max(1,Math.min(30,Number(body?.threshold)||1));
  try{
    ...
    if(all.length<1000)throw new Error(`KOSDAQ 종목이 ${all.length}개만 수집되었습니다.`);
    ...
    return NextResponse.json({results,universeCount:ranked.length,quoteDate:dates.at(-1)||kst(),threshold,source:'Naver Finance KOSDAQ live quote',updatedAt:new Date().toISOString()},{headers:{'Cache-Control':'no-store, max-age=0'}});
  }catch(e){return NextResponse.json({error:e instanceof Error?e.message:'당일 KOSDAQ 시세 수집 실패'},{status:502});}
}
