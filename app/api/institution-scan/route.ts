...
async function getUniverseJson(market:Market,page:number):Promise<Stock[]>{
  ...
  const url=`https://stock.naver.com/api/domestic/market/stock/default?tradeType=KRX&marketType=${marketType}&orderType=marketSum&startIdx=${startIdx}&pageSize=50`;
...
async function getUniverse(market:Market,page:number){
  try{
    ...
    if(stocks.length) return {stocks,source:'Naver JSON'};
  }catch{}
  const stocks=await getUniverseLegacy(market,page);
  return {stocks,source:'Naver PC fallback'};
}

function parseTrendJson(data:any):FlowRow[]{
  const items=Array.isArray(data?.result?.items)?data.result.items:Array.isArray(data?.items)?data.items:[];
  return items.map((x:any)=>{
    const k=x?.krx||x;
    return {
      date:String(x?.localTradedAt ?? x?.date ?? '').replace(/\./g,'-'),
      close:num(k?.closingPrice ?? k?.closePrice),
      rate:num(k?.changeRate ?? k?.fluctuationsRatio),
      volume:num(k?.tradingVolume ?? k?.accumulatedTradingVolume),
      inst:num(k?.organizationNetVolume ?? k?.organPureBuyQuant ?? k?.institutionalValue),
      foreign:num(k?.foreignNetVolume ?? k?.foreignerPureBuyQuant ?? k?.foreignValue),
    };
  }).filter((x:FlowRow)=>/^\d{4}-\d{2}-\d{2}$/.test(x.date));
}

async function getFlowJson(code:string):Promise<FlowRow[]>{
  const urls=[
    `https://m.stock.naver.com/front-api/stock/domestic/trend?code=${code}&page=1&pageSize=20`,
    `https://m.stock.naver.com/front-api/stock/domestic/trend?code=${code}`
  ];
  let best:FlowRow[]=[];
  for(const url of urls){
    try{
      const r=await fetchWithRetry(url,{},1);
      const rows=parseTrendJson(await r.json());
...
async function getFlowLegacy(code:string):Promise<FlowRow[]>{
  ...
  $('table.type2 tr').each((_,el)=>{
    ...
    rows.push({
      date:rawDate.replace(/\./g,'-'), close:num($(td[1]).text()), rate:num($(td[3]).text()),
      volume:num($(td[4]).text()), inst:num($(td[5]).text()), foreign:num($(td[6]).text())
    });
  });
...
async function getFlow(code:string):Promise<{rows:FlowRow[];source:string}>{
  const modern=await getFlowJson(code).catch(()=>[] as FlowRow[]);
  if(modern.length>=15) return {rows:modern,source:'Naver JSON'};
  const legacy=await getFlowLegacy(code).catch(()=>[] as FlowRow[]);
  if(legacy.length>=modern.length && legacy.length) return {rows:legacy,source:'Naver PC fallback'};
  if(modern.length) return {rows:modern,source:'Naver JSON'};
...
function scoreStock(stock:Stock,market:Market,rows:FlowRow[],flowSource:string){
  ...
  if(inst20>0&&foreign20>0) reasons.push('기관·외국인 누적 동시 순매수');
  if(buyDays>=7) reasons.push(`최근 10일 중 ${buyDays}일 수급 우위`);
  if(price20>=-8&&price20<=8) reasons.push(`${r20.length}일 주가 ${price20>=0?'+':''}${price20.toFixed(1)}%로 과열 전`);
  ...
  return {
    market:market==='kospi'?'KOSPI':'KOSDAQ',code:stock.code,name:stock.name,price:latest,score,stage,
    inst5,foreign5,inst20,foreign20,combinedIntensity:Number(combinedIntensity.toFixed(2)),
    price20:Number(price20.toFixed(2)),buyDays,bothDays,absorption,reason:reasons.slice(0,4).join(' · '),
    days:rows.length,flowSource,source:`https://finance.naver.com/item/frgn.naver?code=${stock.code}`
  };
}
...
export async function GET(req:NextRequest){
  ...
  try{
    ...
    const scanned=await pooled(uni.stocks,8,async s=>{
      const flow=await getFlow(s.code);
      return scoreStock(s,market,flow.rows,flow.source);
    });
