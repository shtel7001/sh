...
export const OTP_ID = 'kr-low-screener-all-20260924-reusable-1y';
...
export async function createSessionToken(){
  const now=Date.now();
  const payload={v:2,iat:now,exp:now+SESSION_MS,jti:crypto.randomUUID()};
...
export async function verifySessionToken(token){
  try{
    ...
    return payload&&(payload.v===1||payload.v===2)&&Number(payload.exp)>Date.now();
  }catch{return false;}
}
