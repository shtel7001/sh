...
export async function createSessionToken(){
  const now=Date.now();
  const payload={v:3,iat:now,exp:now+ONE_YEAR_MS,jti:crypto.randomUUID()};
...
export async function verifySessionToken(token){
  try{
    ...
    return payload&&payload.v===3&&Number(payload.exp)>Date.now();
  }catch{return false;}
}
export function otpEqual(a,b){ return safeEq(String(a||''),String(b||'')); }
export const sessionMaxAge = Math.floor(ONE_YEAR_MS/1000);
