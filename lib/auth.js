...
export const OTP_ID = 'low-screener-20260924-1058-v2';
export const SESSION_COOKIE = 'lp_session';
export const AUTH_EXPIRES_AT = Date.parse('2027-09-24T14:59:59.000Z');
...
export function authCodeIsActive(){
  return Date.now() <= AUTH_EXPIRES_AT;
}
export async function createSessionToken(){
  const now=Date.now();
  const exp=Math.min(now+ONE_YEAR_MS,AUTH_EXPIRES_AT);
  const payload={v:2,iat:now,exp,jti:crypto.randomUUID()};
...
export async function verifySessionToken(token){
  try{
    ...
    return payload&&payload.v===2&&Number(payload.exp)>Date.now()&&Date.now()<=AUTH_EXPIRES_AT;
  }catch{return false;}
}
export function otpEqual(a,b){ return safeEq(String(a||''),String(b||'')); }
export function sessionMaxAge(){
  return Math.max(0,Math.floor((AUTH_EXPIRES_AT-Date.now())/1000));
}
